...
def test_search_book(android_management):
    ...
    with step('Проверка, что юзер авторизован'):
        ...
        if browser.element((AppiumBy.ID, 'ru.litres.android:id/circleButtonSubscriptionPaywallClose')
                       ).matching(be.present):
            browser.element((AppiumBy.ID, 'ru.litres.android:id/circleButtonSubscriptionPaywallClose')
                        ).click()
